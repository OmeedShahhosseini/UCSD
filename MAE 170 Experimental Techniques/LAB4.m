...
transfer_vec=zeros(length(f_vec),1); % intitalize transfer function vector
for j=1:length(f_vec) % loop over each frequency to be tested
% create the serial objects% you must replace the port name with the port on your machine
% you can find this through the arduino interface (tools->port)
% the baud rate must match what you selected in your serial read ...% Ardino code
% the first serial object, s_gen, corresponds to the Arduino acting ...% as your signal generator
...
s_gen = serial('COM65','BaudRate',115200);
...
s_read = serial('COM41','BaudRate',115200);
...
% figure(2)
% p = (transfer_vec - min(transfer_vec))/(max(transfer_vec)- min(transfer_vec));
% plot(f_vec,p,'r-o','LineWidth',2,'MarkerSize',4);
% xlabel('frequency (Hz)'); % x-axis label name
end
% Be sure to change filenames if you don’t want to overwrite your data!
save('MAE170_lab4_part3','f_vec_part3','transfer_vec_part3');
% save frequency and gain to mat file
csvwrite('MAE170_lab4part3.csv',[f_vec,transfer]);
% save frequency and gain to csv file
saveas(gcf,'MAE170_lab4_part3');
% save figure